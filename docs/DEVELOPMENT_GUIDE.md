...
8. [后端 API / OpenAPI 契约](#8-后端-api--openapi-契约)
9. [测试约定](#9-测试约定)
10. [Git / 分支 / 提交约定](#10-git--分支--提交约定)
11. [PR 流程](#11-pr-流程)
12. [FAQ](#12-faq)
...
项目用 VSCode 扩展自带的 Ruff（Python lint + format）。

1. VSCode 打开项目，右下角弹"安装推荐扩展"点确定；或手动装 `charliermarsh.ruff`
2. 保存 Python 代码（Ctrl+S）即自动整理 import、格式化、修小毛病
3. 要改 linter 规则去改 [`pyproject.toml`](../pyproject.toml)
...
**Model 新增后**：要在 `app/models/__init__.py` 中 import 过，再重启 API 容器即可（`init_db()` 启动时调 `Base.metadata.create_all`）。需要改字段执行：`make reset && make build`。
...
**查询任务进度**：`GET /api/v1/schedule/schedule-status/{task_id}` → 内部把 Celery state 映射到 `ScheduleStatus`。
...
| 我想看 | 去哪里看 |
|--------|---------|
| 当前所有端点 + 请求/响应 schema | http://localhost:8002/docs |
...
调用方需要按网关约定传入 `X-User-Id` / `X-User-Role`，并处理统一响应包络：
